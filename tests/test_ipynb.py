...
def test_cache_isnt_written_if_no_jupyter_deps_single(
    ...
    monkeypatch.setattr("nila.jupyter_dependencies_are_installed", lambda warn: False)
    result = runner.invoke(
    ...
    jupyter_dependencies_are_installed.cache_clear()
    monkeypatch.setattr("nila.jupyter_dependencies_are_installed", lambda warn: True)
    result = runner.invoke(
        main, [str(tmp_path / "notebook.ipynb"), f"--config={EMPTY_CONFIG}"]
    )
...
def test_cache_isnt_written_if_no_jupyter_deps_dir(
    ...
    tmp_nb.write_bytes(nb.read_bytes())
    monkeypatch.setattr(
        "nila.files.jupyter_dependencies_are_installed", lambda warn: False
    )
    result = runner.invoke(main, [str(tmp_path), f"--config={EMPTY_CONFIG}"])
    assert "No Python files are present to be formatted. Nothing to do" in result.output
    jupyter_dependencies_are_installed.cache_clear()
    monkeypatch.setattr(
        "nila.files.jupyter_dependencies_are_installed", lambda warn: True
    )
    result = runner.invoke(main, [str(tmp_path), f"--config={EMPTY_CONFIG}"])
    assert "reformatted" in result.output
...
def test_unable_to_replace_magics(monkeypatch: MonkeyPatch) -> None:
    src = "%%time\na = 'foo'"
    monkeypatch.setattr("nila.handle_ipynb_magics.TOKEN_HEX", lambda _: "foo")
    with pytest.raises(
